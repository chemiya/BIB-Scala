...
for (nombre_columna <- listaAtributosNumericos) {
  ...
  // Distribución de los valores del atributo
...
// Análisis atributos categóricos-------------------------------------------------------------------
// Cuantos valores diferentes puede tomar
def numero_diferentes(nombre_columna:String): Long = {
    val cuenta=census_df.select(nombre_columna).distinct().count()
    cuenta
  }

// Extraer cada uno de los valores diferentes
...
// Agrupa y cuenta cuantos valores de cada uno
...
// Correlación atributos numéricos---------------------
...
for (i <- 0 until listaAtributosNumericos.length) {
  ...
  val columna_actual = listaAtributosNumericos(i)
    ...
    for (j <- (i + 1) until listaAtributosNumericos.length) {
        ...
        // Extraer valor de la posición 0,1
        val valor = correlacionMatrix.apply(0, 1)
        println("Correlacion entre "+columna_actual+" con "+siguiente_columna+" es: "+valor)
    }
}
...
// Correlación categóricos-------------------------------------------------------------
var correlacion_categoricas=Array[String]()
...
for (i <- 0 until listaAtributosCategoricos.length) {
  val columna_actual = listaAtributosCategoricos(i)
    ...
    for (j <- (i + 1) until listaAtributosCategoricos.length) {
      ...
      println(fila)
      correlacion_categoricas = correlacion_categoricas :+ fila
      val nombre=columna_actual+"-"+siguiente_columna
      tablaContingencia.write.mode("overwrite").csv(nombre)
    }
}

sc.parallelize(correlacion_categoricas.toSeq,1).saveAsTextFile("correlacion_categoricas")
